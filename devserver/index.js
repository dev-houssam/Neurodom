...
// devserver/ is at root level, packages/ is a sibling

const CORE_PATH      = path.resolve(__dirname, "../packages/core");
const COMPILER_PATH  = path.resolve(__dirname, "../packages/compiler");
const DEVTOOLS_PATH  = path.resolve(__dirname, "../packages/devtools");
...
async function startDevServer() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // /packages/... → project root / packages /...
    ...
    // Everything else → serve from cwd (the project where devserver was launched)
    else {
...
function buildRuntimeBundle() {
  ...
  files.forEach((f) => {
    try {
      ...
      // 1. Remove all import lines entirely
      code = code.replace(/^import\s[\s\S]*?from\s+['"][^'"]+['"]\s*;?\r?\n/gm, "");

      // 2. Remove "export { ... };" re-export lines entirely
      code = code.replace(/^export\s+\{[^}]*\}\s*(?:from\s+['"][^'"]+['"])?\s*;?\r?\n/gm, "");

      // 3. Remove "export default " and "export " prefixes (keep the declaration)
...
async function compileNRDFile(content) {
  const { compileNRD } = await import(path.join(COMPILER_PATH, "index.js"));
  const def = compileNRD(content);

  // Sérialiser les flows : les fonctions (filter/map/condition/raw) sont
  // converties en chaîne puis réinsérées comme code JS valide.
  function serializeStep(step) {
    const s = { ...step };
    // Convertir toutes les valeurs Function en string pour JSON
    for (const key of Object.keys(s)) {
      if (typeof s[key] === "function") {
        s[key] = `__FN__${s[key].toString()}__FN__`;
      }
    }
    // "cases" dans match contient des tableaux de steps
    if (s.cases) {
      const cases = {};
      for (const [k, v] of Object.entries(s.cases)) {
        cases[k] = v.map(serializeStep);
      }
      s.cases = cases;
    }
    return s;
  ...
  function serializeFlow(flow) {
    return {
      name: flow.name,
      pipelines: flow.pipelines.map(p => ({
        ...p,
        steps: p.steps.map(serializeStep),
        ...(p.condition ? { condition: `__FN__${p.condition.toString()}__FN__` } : {}),
      })),
    };
  }

  // Générer le JSON des flows et remplacer les marqueurs __FN__...__FN__
  // par du vrai code JS (sans les guillemets JSON autour)
  let flowsJson = JSON.stringify(def.flows.map(serializeFlow), null, 2);

  // Remplace "__FN__function(...){...}__FN__" par la fonction sans les guillemets
  // Le JSON met les fonctions entre guillemets doubles avec échappement
  flowsJson = flowsJson.replace(/"__FN__([\s\S]*?)__FN__"/g, (_, fnBody) => {
    // Désescaper les caractères JSON (\n, \", \\, etc.)
    const unescaped = JSON.parse('"' + fnBody + '"');
    return unescaped;
  });

  return `// Auto-compiled .nrd — NeuroDOM Compiler
export default {
  ...
  state: ${def.state ? def.state.toString() : "() => ({})"},
  flows: ${flowsJson},
...
function injectHMR(html) {
  // Inject ONLY the HMR websocket client.
  // The runtime is loaded via native ES module imports in main.js — no bundle needed.
  const scripts = `  <script src="/__neurodom__/hmr-client.js"></script>\n`;
